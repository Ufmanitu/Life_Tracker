// ============================================================
//  duck-poses.js -- Life Tracker Duck Companion
//  Step 2: 8 emotional SVG poses for the duck character
//
//  Each entry is a self-contained inline SVG string (viewBox 0 0 200 200,
//  transparent background, no fixed pixel size) meant to be dropped into
//  a DOM node via innerHTML and swapped whenever Duck.getEmotion() changes
//  (see Step 3's DuckUI.render() and Step 4's notifyUI()).
//
//  All fill/stroke colors reference CSS custom properties (--duck-*) so the
//  palette lives in ONE place (style.css) instead of being baked into 8
//  separate SVG strings. Paste the DUCK_PALETTE_CSS block below into
//  style.css :root (Step 3) before rendering these.
//
//  Design notes:
//   - Body/head/wings/feet/shadow are IDENTICAL across all 8 poses (same
//     coordinates) so swapping emotions never makes the duck "jump" or
//     resize -- only eyes, brows, beak and small accent props differ.
//   - The duck's own colors are intentionally NOT theme-dependent (unlike
//     --bg/--text/etc. which flip per theme). Like a real pet, Quackers
//     looks the same in every room. Two exceptions read the surrounding
//     theme on purpose: the "z" in the tired pose uses var(--text-muted),
//     and the ground shadow uses var(--duck-shadow) so it stays a soft
//     fixed dark tone regardless of theme.
//   - duck.js (Step 1) defines 13 EMOTIONS but this step only covers the
//     8 the brief asked for. EMOTION_FALLBACK below maps the other 5 to
//     the closest existing pose as a placeholder; Step 8 ("Personality,
//     animations & polish") is the planned place to give content/excited/
//     working/hungry/celebrating their own bespoke poses (confetti for
//     celebrating, empty-bowl for hungry, etc).
// ============================================================

// ----------------------------------------------------------------
//  PALETTE -- paste into style.css :root{} (Step 3)
//  One fixed identity for the duck, independent of the active theme.
// ----------------------------------------------------------------
const DUCK_PALETTE_CSS = `
  --duck-body:         #ffc93c;  /* main feather yellow */
  --duck-body-light:   #ffe082;  /* head highlight */
  --duck-body-dark:    #f2a93c;  /* wings, tail, brow material, shading */
  --duck-belly:        #fff6da;  /* pale cream belly patch */
  --duck-beak:         #ff9d3d;  /* beak top */
  --duck-beak-dark:    #e8762a;  /* beak underside / nostrils */
  --duck-feet:         #ff9d3d;  /* webbed feet */
  --duck-eye:          #2b1d12;  /* warm near-black, softer than pure #000 */
  --duck-cheek:        #ff8fa3;  /* happy/eating/drinking blush -- echoes the app's pink accent */
  --duck-cheek-sick:   #a5d6a7;  /* pale green blush when sick */
  --duck-cheek-angry:  #ff7a59;  /* flushed cheeks when grumpy */
  --duck-accent-blue:  #4f6ef7;  /* water droplets, tears -- matches the app's primary blue accent */
  --duck-accent-sleep: #b39ddb; /* Zzz text + moon, soft lavender for night */
  --duck-sick-tint:    #8bc34a; /* faint green overlay wash when sick */
  --duck-anger:        #e64a3b; /* grumpy motion marks */
  --duck-crumb:        #e8b06a; /* food crumbs while eating */
  --duck-shadow:       rgba(0,0,0,.18); /* ground shadow under feet, fixed for all themes */
`;
...
const DUCK_POSES = {
  ...
  // Default contented/joyful pose - used by resolveEmotion() for mood>70, and as the post-trigger emotion for habitDone, taskDone, pillTaken, pomodoroFinish, morningBoost.
  happy: `
...
`,
  ...
  // Low mood pose - used for mood<30, and for pillMissed.
  sad: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
`,
  ...
  // Low energy / sleepy-but-awake pose - used for mood 30-50 or energy<35, workoutRecovering, and as the "tired" stage of workoutDone before it flips to happy.
  tired: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
`,
  ...
  // Used for health<25 and for cyclePhase==='period' (periodDay trigger).
  sick: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
`,
  ...
  // Used for cyclePhase==='pms' (pmsPhase trigger).
  grumpy: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
`,
  ...
  // Transient pose for foodLogged (4s) - duck visibly eats when a meal is logged.
  eating: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
`,
  ...
  // Transient pose for waterLogged (3s).
  drinking: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
`,
  ...
  // Used for energy<15 (deep energy depletion) - distinct from "tired" which is alert-but-low.
  sleeping: `
<svg viewBox="0 0 200 200" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <ellipse cx="100" cy="186" rx="46" ry="7" fill="var(--duck-shadow)"/>
  <ellipse cx="78" cy="180" rx="13" ry="6" fill="var(--duck-feet)"/>
...
// ----------------------------------------------------------------
//  FALLBACK MAP -- for the 5 duck.js EMOTIONS not yet covered here.
//  Replace these with bespoke poses in Step 8.
// ----------------------------------------------------------------
const EMOTION_FALLBACK = {
  content:     'happy',
  excited:     'happy',   // TODO Step 8: sparkles + lifted wings
  working:     'tired',   // TODO Step 8: furrowed-focus pose, pencil/checkmark prop
  hungry:      'sad',     // TODO Step 8: empty bowl, drooping antenna-line "hunger waves"
  celebrating: 'happy',   // TODO Step 8: confetti + party hat, reuse "excited" energy
};

// Single entry point Step 3/4 should use -- never index DUCK_POSES directly.
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { DUCK_POSES, DUCK_PALETTE_CSS, EMOTION_FALLBACK, getDuckPoseSVG };
}